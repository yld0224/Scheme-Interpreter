...
struct ValueBase {
    ...
    virtual ~ValueBase() = default;//虚析构函数，指针可以向下转型
};//value的基类 
...
Assoc empty();
Assoc extend(const std::string&, const Value &, Assoc &);//增加一个var
void modify(const std::string&, const Value &, Assoc &);//改变某个var的值
Value find(const std::string &, Assoc &);//在当前环境寻找该var
